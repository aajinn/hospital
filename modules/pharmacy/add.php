...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($errors)) {
        ...
        $checkQuery = "SELECT id FROM medicines WHERE name = ? AND batch_number = ?";
        $checkStmt = $connection->prepare($checkQuery);
        $checkStmt->bind_param('ss', $name, $batch_number);
        $checkStmt->execute();
    ...
    // If no errors, insert the medicine
    if (empty($errors)) {
        $medicineData = [
            'name' => $name,
            'category' => $category,
        ...
        $medicineId = insertRecord('medicines', $medicineData);
        
        if ($medicineId) {
            logActivity('Medicine Added', "Medicine: $name (ID: $medicineId)");
            redirectWithMessage('index.php', 'Medicine added successfully!', 'success');
        } else {
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="page-header">
                            <h1><i class="fas fa-plus"></i> Add Medicine</h1>
                                        ...
                                        <div class="col-md-4">
                                            <div class="mb-3">
                                                ...
                                                <input type="number" class="form-control" id="min_quantity" name="min_quantity" 
                                                       value="<?php echo htmlspecialchars($_POST['min_quantity'] ?? '10'); ?>" 
                                                       min="0" required>
    ...
    <script>
        ...
        document.getElementById('addMedicineForm').addEventListener('submit', function(e) {
            ...
            if (expiryDate) {
                ...
                if (expiry < today) {
                    if (!confirm('The expiry date is in the past. Are you sure you want to add this medicine?')) {
                        e.preventDefault();
                        return;
                    }
                }
