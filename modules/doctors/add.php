...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($errors)) {
        $existingDoctor = selectRecords('doctors', ['phone' => $formData['phone']]);
        if (!empty($existingDoctor)) {
    ...
    if (empty($errors) && !empty($formData['email'])) {
        $existingDoctor = selectRecords('doctors', ['email' => $formData['email']]);
        if (!empty($existingDoctor)) {
            $errors[] = 'A doctor with this email address already exists';
        }
    }
    
    // If no errors, save doctor
    if (empty($errors)) {
        try {
            ...
            $insertId = insertRecord('doctors', $doctorData);
            
            if ($insertId) {
                // Log activity
                logActivity('Doctor Added', "New doctor registered: {$formData['name']} - {$formData['specialization']}");
                
                // Redirect with success message
                redirectWithMessage('index.php', "Doctor registered successfully!", 'success');
            } else {
                $errors[] = 'Failed to register doctor. Please try again.';
            }
        } catch (Exception $e) {
            error_log("Doctor registration error: " . $e->getMessage());
            $errors[] = 'An error occurred while registering the doctor. Please try again.';
        }
    }
}
        ...
        <div class="col-md-10 main-content">
            <div class="content-header">
                <div class="d-flex justify-content-between align-items-center">
                    <div>
                        <h1 class="h3 mb-0">Add New Doctor</h1>
